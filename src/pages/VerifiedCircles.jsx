...
const VerifiedCircles = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            console.error('Error fetching verified circles:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (actionId) return;
        if (!window.confirm('Are you sure you want to remove this verified circle?')) return;
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Verified Circle Library</h1>
                    <p className="text-gray-500 mt-1 font-medium">Browse all verified events and communities available on the platform</p>
                </div>
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden min-h-[500px]">
                <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className="px-4 py-2 bg-green-50 text-green-700 rounded-xl border border-green-200 text-sm font-black uppercase tracking-wider">
                            Verified Only
                        </div>
                    </div>
                ...
                <div className="p-8">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-4">
                            ...
                            <p className="text-gray-500 font-bold text-lg animate-pulse">Scanning library...</p>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="text-center py-32 bg-gray-50/50 rounded-3xl border-4 border-dashed border-gray-200">
                            <Globe size={100} className="text-gray-100 mx-auto mb-6" />
                            <h3 className="text-2xl font-bold text-gray-900">No {activeTab} in Library</h3>
...
export default VerifiedCircles;
